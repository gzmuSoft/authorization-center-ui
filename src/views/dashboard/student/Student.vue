<template lang="pug">
  v-card.student.mt-12
    ...
    v-card-text
      skeleton-student(:init="init", :num="8")
...
import SkeletonStudent from '@/components/skeleton-loader/SkeletonStudent.vue'

@Component({ components: { SkeletonStudent, CardHeader, StudentView } })
export default class Student extends Mixins(FormValidateMixin) {
  ...
  async getData (classId) {
    this.loading = true
    try {
      const res = await studentPage({ classId })
      ...
      if (res.data.hasOwnProperty('classes')) {
        ...
        if (this.classIndex === -1 && this.classes.length > 0) {
          this.classIndex = 0
          await this.getData(this.classes[this.classIndex].id)
        }
      }
    } finally {
      this.loading = false
